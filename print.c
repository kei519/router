...
int PrintIpHeader(struct iphdr *iphdr, u_char *option, int optionLen, FILE *fp)
{
	...
	fprintf(fp, "tos_len=%u,", ntohs(iphdr->tot_len));
...
int PrintIcmp6(struct icmp6_hdr *icmp6, FILE *fp)
{
	...
	fprintf(fp, "icmp6_typ=%u", icmp6->icmp6_type);
	if (icmp6->icmp6_type == 1) {
		fprintf(fp, "(Destination Unreacheable),");
...
int PrintTcp(struct tcphdr *tcphdr, FILE *fp)
{
	...
	fprintf(fp, "doff=%u,", ntohs(tcphdr->doff));
	fprintf(fp, "urg=%u,", ntohs(tcphdr->urg));
	fprintf(fp, "ack=%u,", ntohs(tcphdr->ack));
	fprintf(fp, "psh=%u,", ntohs(tcphdr->psh));
	fprintf(fp, "rst=%u,", ntohs(tcphdr->rst));
	fprintf(fp, "syn=%u,", ntohs(tcphdr->syn));
	fprintf(fp, "fin=%u,", ntohs(tcphdr->fin));
